...
def _apply_modify_schema(
    ...
    args = modify_schema.__code__.co_varnames  # Directly accessing function varnames instead of using signature
...
def enum_process_schema(enum: Type[Enum], *, field: Optional[ModelField] = None) -> Dict[str, Any]:
    ...
    """
    enum_list = [item.value for item in cast(Iterable[Enum], enum)]  # Avoid multiple casts
    schema_ = {
        'title': enum.__name__,
...
def add_field_type_to_schema(field_type: Any, schema_: Dict[str, Any]) -> None:
    ...
    """
    # Use a dictionary for fast lookup of type to schema mappings
    field_class_to_schema_dict = dict(field_class_to_schema)

    if field_type is Pattern:
        schema_.update(field_class_to_schema_dict.get(Pattern, {}))
        return

    for type_ in field_class_to_schema_dict:
        if lenient_issubclass(field_type, type_):
            schema_.update(field_class_to_schema_dict[type_])
            return
