...
def _apply_modify_schema(
    ...
    if 'field' in modify_schema_args or 'kwargs' in modify_schema_args:
...
def enum_process_schema(enum: Type[Enum], *, field: Optional[ModelField] = None) -> Dict[str, Any]:
    ...
    """
    # Prepares enum schema with basic information
    schema_ = {
        'title': enum.__name__,
    ...
    if modify_schema:
        # Use precomputed signature args
        modify_schema_args = _get_modify_schema_signature(modify_schema)
        _apply_modify_schema(modify_schema, field, schema_, modify_schema_args)
...
# Precompute signatures for modify schema functions
def _get_modify_schema_signature(modify_schema: Callable[..., None]) -> set:
    return set(signature(modify_schema).parameters.keys())
