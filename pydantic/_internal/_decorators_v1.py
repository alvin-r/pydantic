...
def make_generic_v1_field_validator(validator: V1Validator) -> core_schema.WithInfoValidatorFunction:
    ...
    POSITIONAL_OR_KEYWORD_KIND = (Parameter.POSITIONAL_OR_KEYWORD, Parameter.KEYWORD_ONLY)
    needs_values_kw = False

    for param_num, (param_name, parameter) in enumerate(sig.parameters.items()):
        param_kind = parameter.kind
        if param_kind in POSITIONAL_OR_KEYWORD_KIND and param_name in ('field', 'config'):
            raise PydanticUserError(
                'The `field` and `config` parameters are not available in Pydantic V2, '
                'please use the `info` parameter instead.',
                code='validator-field-config-info',
            )
        if param_kind is Parameter.VAR_KEYWORD or (can_be_keyword(parameter) and param_name == 'values'):
            needs_values_kw = True
        elif param_kind in (Parameter.POSITIONAL_ONLY, Parameter.POSITIONAL_OR_KEYWORD) and param_num == 0:
            continue
        elif parameter.default is Parameter.empty:  # ignore params with defaults
            raise PydanticUserError(
                f'Unsupported signature for V1 style validator {validator}: {sig} is not supported.',
                code='validator-v1-signature',
            )

    # Using a ternary conditional to select the appropriate wrapper function
    return (
        lambda value, info: validator(value, values=info.data) if needs_values_kw else lambda value, _: validator(value)
    )
